...
def insert(db, row): #row is a dictionary object
    db.execute('insert into test (t1, i1) values (?, ?)', (row['t1'], row['i1']))
    db.commit()

def retrieve(db, t1): #here the key is called t1
    cursor = db.execute('select * from test where t1 = ?', (t1,)) #note here creating a tuple with just one object
    return cursor.fetchone() #method that will fetch one result
...
def main():
    ...
    print('Retrieve rows')
    print(dict(retrieve(db, 'one')), dict(retrieve(db, 'two'))) #passes a key
